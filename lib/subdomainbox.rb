module ActionController
  class Base
    ...
    class SubdomainboxIDViolation < StandardError
    end
    ...
    def subdomainbox_no_subdomain_match!(allowed)
      if request.format == 'text/html'
        if request.get?
          ...
          allowed_id_name = allowed_id_name.to_sym if allowed_id_name
        ...
        if allowed_id_name
          if id = request.subdomain.sub(/\A#{allowed_subdomain}\.?/, '')
            if params.keys.include?(allowed_id_name)
              raise SubdomainboxIDViolation.new unless id == params[allowed_id_name]
            else
